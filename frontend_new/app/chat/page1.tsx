...
function ChatPage() {
  ...
  useEffect(() => {
    ...
    if (jsonFile) {
      ...
      const snippet = currentStreamingState?.currentJsonSnippet ?? "";
      setJsonContent(extractFenced(snippet, "json"));
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    if (pyFile) {
      ...
      const snippet = currentStreamingState?.pythonSnippet ?? "";
      setPyContent(extractFenced(snippet, "python"));
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
        ...
        <Panel defaultSize={40} minSize={22}>
          <aside style={{ width: "100%", display: "flex", flexDirection: "column", background: "var(--panel)", borderLeft: "0.5px solid var(--border)", height: "100%", minWidth: 0, overflow: "hidden" }}>
            ...
            {rightMode === "code" ? (
              ...
              currentSessionId ? (
                <PreviewPane
                  previewUrl={currentStreamingState?.previewUrl}
                  sessionId={currentSessionId}
                  flowState={flowState}
                  endpointDefault={currentStreamingState?.endpointDefault}
